...
import mapClasses.BSTMap.MapEntry;
...
public class BSTMap<K, V> implements Map<K, V> {
	...
	public V put(K key, V value) {
		...
		if(get(key)==null) { //verifica si no es null
			tree.addElement(newEntry); // si es null, it adds it to the tree
			return null;
		}
		else {
			Position<Entry<K,V>> pos = tree.getPosition(new MapEntry<>(key,get(key))); //get the position
			V value2 = pos.getElement().getValue(); //get the value of the element that already existed in that position
			pos.setElement(newEntry); //set the new element in place
			return value2; // return element that was eliminated from the position
		}
	...
	public V remove(K key) {
		...
		// them to instance field tree.... EXERCISE 1
		//...
		Position<Entry<K,V>> p = tree.getPosition(new MapEntry<>(key, get(key))); //get position
		if(get(key) != null && tree.isExternal(p)) { //check is the pisition is null if not continue on
			V value2 = p.getElement().getValue(); // get the value
			
			tree.remove(p); //remove from the tree
			return value2; //return the deleted value
		}
		return null; //if no position found then return null cause i didn't remove anything
	...
	public Iterable<K> keySet() {
		...
		//...
		ArrayList<K> klist = new ArrayList<>(); //creates list to save the keys
		for(Entry<K,V> s: entrySet()) {  //sort through the keyset
			klist.add(s.getKey());
		}
		return klist; //returns complete list containing all keys
	}
	...
	public Iterable<V> values() {
		...
		// inner class......  EXERCISE 2
		//...
		ArrayList<V> vlist = new ArrayList<>(); //creates list to save the keys
		for(Position<Entry<K,V>> pos: tree.positions()) { //traverse
			vlist.add(pos.getElement().getValue());//get the elements
		}
		return vlist; //return list with all values.
	}
